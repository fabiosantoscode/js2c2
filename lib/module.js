...
global.Module = (function() {
  var evalWithFilename = process.binding('eval_with_filename')
  ...
  var coreModules = File.list('node/lib')
    .map(function(file) {
      if (/\.js$/.test(file)) return file.replace(/\.js$/, '')
    })
    .filter(Boolean)
    ...
    function resolve(filename, isRecursive) {
      ...
      if (/^\.\//.test(filename) && !/^\.\/\.\//.test(filename)) {
        ...
        for (
          ...
          var foundNodeModule = File.list(
            currentDir + '/node_modules/' + moduleName
          )
          if (foundNodeModule && foundNodeModule.indexOf('package.json') >= 0) {
            if (moduleName !== filename) {
              // require('moduleName/foo/bar')
              return resolve(
                currentDir + '/node_modules/' + filename,
                resolveIsRecursive
              )
            }
            var packageJson = JSON.parse(
              File.read(
                currentDir + '/node_modules/' + moduleName + '/package.json'
              ) || 'null'
            )
            if (packageJson && packageJson.main) {
              return finaliseResolve(
                currentDir +
                  '/node_modules/' +
                  moduleName +
                  '/' +
              ...
              return finaliseResolve(
                currentDir + '/node_modules/' + moduleName + '/index.js'
              )
            }
          }
        }
  ...
  Module.prototype._run = function _runModule() {
    ...
    var massagedCode = global._hacks.massageRegex(
      PREFIX + code + SUFFIX,
      this._filename
    )

    var moduleFn = evalWithFilename(massagedCode, this._filename)
